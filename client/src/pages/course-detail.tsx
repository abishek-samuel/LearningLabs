  ...
  return (
    <div className="flex items-center">
      {[...Array(displayFullStars)].map((_, i) => (
        <StarIcon
          key={`full-${i}`}
          className={cn(size, "text-yellow-400 fill-yellow-400")}
        />
      ))}
      {[...Array(5 - displayFullStars)].map((_, i) => (
        <StarIcon
          key={`empty-${i}`}
          className={cn(size, "text-slate-300 dark:text-slate-600")}
        />
      ))}
    </div>
  );
...
export default function CourseDetail() {
  ...
  const { data: enrollments = [] } = useQuery<any[]>({
    ...
    queryFn: async () => {
      ...
      if (!res.ok) {
        console.error("Failed fetch enrollments");
        return [];
      }
      return res.json();
    },
    ...
    queryFn: async () => {
      ...
      return {
          ...
          ? {
              ...
              avatar:
                ...
                `https://ui-avatars.com/api/?name=${
                  data.instructor.firstName?.[0] || "N"
                }+${data.instructor.lastName?.[0] || "A"}`,
              ...
              profilePicture: data.instructor.profilePicture,
            }
          : {
              id: 0,
        ...
        modules: (data.modules || [])
          .sort(
            (a: Module, b: Module) => (a?.position ?? 0) - (b?.position ?? 0)
          )
          .map((mod: Module) => ({
            ...mod,
            lessons: (mod.lessons || [])
              ...
              .map((lesson: Lesson) => ({
                ...
                duration: lesson.duration
                  ? `${Math.floor(Number(lesson.duration) / 60)}:${(
  ...
  const { data: enrollmentCount = 0, isLoading: isLoadingEnrollmentCount } =
    useQuery<number>({
      ...
      queryFn: async (): Promise<number> => {
        ...
        try {
          const response = await fetch(
            `/api/courses/${courseId}/enrollments/count`
          );
          if (!response.ok) {
            console.error(
  ...
  const { data: avgRatingData, isLoading: isLoadingAvgRating } =
    useQuery<AverageRatingResponse>({
      queryKey: ["reviewAverage", courseId],
      queryFn: async () => {
        if (!courseId) return { averageRating: 0, reviewCount: 0 };
        const response = await fetch(
          `/api/courses/${courseId}/reviews/average`
        );
        if (!response.ok) {
      ...
      return (Array.isArray(data) ? data : []).map((review: any) => ({
        ...
        user: {
          ...
          profilePicture:
            ...
            }+${review.user?.lastName?.[0] || "A"}&background=random`,
        },
      }));
    },
    enabled: !!courseId,
  ...
  const lessonCompletionMap = useMemo(
    () =>
      lessonProgressData.reduce((acc: Record<number, boolean>, p) => {
        if (p?.lessonId != null) {
  ...
  const enrollMutation = useMutation({
    ...
    onSuccess: (data, courseIdToEnroll) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["course", courseIdToEnroll.toString()],
      });
      queryClient.invalidateQueries({
        queryKey: ["enrollmentCount", courseIdToEnroll.toString()],
      });
  ...
  const handleNavigateToLesson = (moduleId: number, lessonId: number) => {
    ...
    setLocation(
      `/course-content?id=${course.id}&moduleId=${moduleId}&lessonId=${lessonId}`
    );
  };
  ...
  const getLessonIcon = (lesson: Lesson) => {
    ...
    if (type === "assessment" || lesson.questions)
      return (
        <ClipboardCheck
          className={cn(
            "mr-3 h-5 w-5",
  ...
  // --- Render Logic ---
  const isLoading =
    isLoadingCourse || isLoadingEnrollmentCount || isLoadingAvgRating;
      ...
      <MainLayout>
        <div className="px-4 sm:px-6 lg:px-8 py-8 max-w-7xl mx-auto text-center">
          ...
          <p className="mt-2 text-slate-500">
            {courseError?.message ||
              "The course you are looking for could not be loaded."}
          </p>
          <Button
  ...
  return (
    <MainLayout>
      ...
      <div className="bg-white dark:bg-slate-900 shadow">
        {" "}
        {/* Reverted to original shadow */}
        <div className="px-4 sm:px-6 lg:px-8 py-6">
          {" "}
          {/* Reverted padding/max-width */}
          ...
          <div className="md:flex md:items-start md:justify-between">
            <div className="min-w-0">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-x-6 gap-y-2 text-sm">
                ...
                <div
                  className="flex items-center gap-1"
                  title={`Average Rating: ${displayRating.toFixed(1)} out of 5`}
                >
            ...
            <div className="mt-4 flex gap-3 md:mt-0 md:ml-0">
              {" "}
              {/* Reverted margin */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <Tabs defaultValue="content" className="w-full">
              <TabsList className="mb-6 w-full grid grid-cols-3">
                {" "}
                {/* Reverted to original grid/border/style */}
                                      ...
                                      lesson?.id != null && (
                                        <div // ** CORRECTED: Use div and ensure onClick is passed **
                                          ...
                                          className={cn(
                                            ...
                                            isEnrolled
                                              ...
                                              : "cursor-not-allowed opacity-70" // Adjust hover based on enrollment
                                          )}
                                          onClick={
                                            isEnrolled
                                              ? () =>
                                        ...
                                        >
                                          ...
                                          {lessonCompletionMap[lesson.id] ? (
                                            <Badge
                                              variant="outline"
                                              className="bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400 border-green-200 dark:border-green-900/50 flex-shrink-0"
                                            >
                                              {" "}
                                              Completed{" "}
                                            </Badge>
                                            ...
                                            ) // tabIndex=-1 as div handles click
                                          )}
                                        </div>
                                      )
                                  )}
                  ...
                  <Card>
                    {/* <CardHeader>
                      <CardTitle>
                        ...
                        <span
                          className="px-2 py-0.5 text-[10px] font-bold text-white bg-gradient-to-r from-purple-500 to-pink-500 rounded-full shadow-sm"
                          style={{
                          ...
                          {reviewsData.map(
                            (review) =>
                              review?.id != null && (
                                <div
                                  key={review.id}
                                  className="flex items-start gap-4 pt-6 first:pt-0"
                                >
          ...
          <div>
            <div className="lg:sticky lg:top-6 space-y-6">
              ...
              {isEnrolled &&
                course.progress >= 0 && ( // Use original check >= 0
                  <Card>
                    <CardHeader>
                    ...
                    <CardContent>
                      <div className="space-y-2">
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400 mt-4">
                          You're making great progress! Continue where you left
                          off:
                        </p>
                        <Button
                          onClick={handleContinueProgress}
                          className="bg-primary dark:bg-blue-600 dark:hover:bg-blue-700 w-full mt-4 text-white"
                        >
                          <Play className="mr-2 h-4 w-4" />
                          Continue Course
                        </Button>{" "}
                        {/* Use mt-4 if original */}
              ...
              <Card>
                ...
                <CardFooter>
                  {isEnrolled ? (
                    ...
                    <Button
                      onClick={handleContinueProgress}
                      className="w-full bg-primary dark:bg-blue-600 dark:hover:bg-blue-700 text-white"
                    >
                      Continue Learning
                    </Button>
                  ) : (
