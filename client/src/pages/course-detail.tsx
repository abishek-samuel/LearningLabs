...
export default function CourseDetail() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      return {
        ...
        rating: 4.5,
        reviews: 0,
        enrolled: 0, // Placeholders
        prerequisites: data.prerequisites || [],
        objectives: data.objectives || [],
        modules: (data.modules || [])
          .sort((a: Module, b: Module) => a.position - b.position)
          .map((mod: Module) => ({
            ...mod,
            lessons: (mod.lessons || [])
  ...
  const lessonCompletionMap = lessonProgressData.reduce(
  ...
  const enrollMutation = useMutation({
    ...
    onSuccess: (data, courseIdToEnroll) => {
      ...
      }); // Refetch course details to update progress potentially
      ...
      if (course) {
        ...
        if (firstModule && firstLesson) {
          ...
          toast({
            title: "Enrolled",
            description:
              "Enrolled successfully, but couldn't find the first lesson.",
            variant: "default",
          });
        }
      }
  ...
  const handleContinueProgress = () => {
    ...
    if (firstModule && firstLesson) {
      ...
      toast({
        title: "Error",
        description: "Could not find the first lesson to continue.",
        variant: "destructive",
      });
    }
  };
  ...
  const handleNavigateToLesson = (moduleId: number, lessonId: number) => {
    if (course?.id) {
      ...
      toast({
        title: "Error",
        description: "Cannot navigate to lesson.",
        variant: "destructive",
      });
    }
  };
  ...
  if (isError || !course) {
    // Use the correctly destructured isError
    ...
    return (
      <MainLayout>
        <div className="px-4 sm:px-6 lg:px-8 py-8 max-w-7xl mx-auto text-center">
          ...
          {/* Use the correctly destructured error */}
          <Button
